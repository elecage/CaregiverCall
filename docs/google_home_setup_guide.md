...
CaregiverCall은 자동화를 2개로 나누어 구성합니다. 첫 번째 자동화는 버튼을 눌렀을 때 보호자에게 호출을 알리고, 호출자에게는 보호자에게 알림을 보냈다는 방송을 내보냅니다. 두 번째 자동화는 보호자가 호출을 확인했을 때 직접 실행하는 루틴이며, 호출기를 `꺼짐` 상태로 되돌리고 호출자에게 보호자가 확인했다는 방송을 내보냅니다.

### 자동화 1: 호출 알림

자동화 1의 이름은 `보호자 연락 알림`으로 설정합니다.

![Caregiver alert automation](assets/google_home_setup/24-caregiver-alert-automation.jpg)

1. 새 자동화를 만들고 시작 조건을 추가할 때 `기기`를 선택합니다. 이 자동화는 CaregiverCall 버튼 상태를 시작 조건으로 사용합니다.
...
3. 상태 선택 창에서 `켜짐`을 선택합니다. 버튼을 눌러 호출이 발생한 상태를 `켜짐`으로 보고, 이 상태가 되면 보호자 알림 자동화가 실행되도록 합니다.
...
5. 작업에는 두 가지를 추가합니다. 보호자 휴대전화로 보내는 알림을 먼저 추가하고, 이어서 Google Home 스피커 방송을 추가합니다. 캡처 흐름에서는 방송 작업을 추가하기 위해 `방송`을 선택합니다.

![Automation actions](assets/google_home_setup/22-automation-actions.jpg)

6. 방송 작업에는 호출자에게 들려줄 문구와 방송할 기기 또는 방을 설정합니다. 예: `보호자에게 알림을 보냈습니다. 잠시만 기다려주세요.`

![Device state action](assets/google_home_setup/23-device-state-action.jpg)

7. 자동화 편집 화면에서 시작 조건과 작업이 모두 들어간 것을 확인한 뒤 저장합니다. 이후 CaregiverCall 버튼을 눌러 보호자 알림과 호출자 안내 방송이 실제로 실행되는지 확인합니다.

### 자동화 2: 보호자 확인

1. 두 번째 자동화는 보호자가 Google Home 앱이나 음성 명령으로 직접 실행하는 루틴으로 만듭니다. 시작 조건은 기기 상태가 아니라 보호자가 실행할 수 있는 수동 루틴 또는 음성 명령으로 설정합니다. 예: `호출 확인`.

2. 작업에는 `보호자호출기`를 `꺼짐`으로 변경하는 기기 제어를 추가합니다. 이 동작은 호출 상태를 초기화해서 다음 호출을 받을 수 있게 합니다.

3. 이어서 Google Home 스피커 방송을 추가합니다. 방송 문구는 호출자에게 확인 사실이 전달되도록 작성합니다. 예: `보호자가 확인했습니다. 곧 도착합니다.`

4. 자동화 편집 화면에서 이름이 `호출확인`으로 되어 있고, 시작 조건과 작업이 모두 들어간 것을 확인한 뒤 저장합니다.

![Automation done](assets/google_home_setup/24-automation-done.jpg)

5. 저장 후에는 버튼을 눌러 첫 번째 자동화가 실행되는지 확인하고, 보호자가 두 번째 루틴을 실행했을 때 호출기가 `꺼짐`으로 돌아가며 확인 방송이 나오는지 확인합니다.
